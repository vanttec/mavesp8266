...
async def scan_network(network: ipaddress.ip_network):
    async with aiohttp.ClientSession() as async_session:
        tasks = [
            asyncio.create_task(
                coro=check_host(async_session=async_session, address=address)
            )
            for address in network
        ]
        host_list = dict(
            zip(list(network), await asyncio.gather(*tasks, return_exceptions=False))
        )
        # Only get addresses with 200 status code response
        filtered_list = dict(
            filter(
                lambda item: item[1] is not None and item[1][0] == 200,
                host_list.items(),
            )
        )
